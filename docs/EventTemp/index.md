...
```mcfunction
# 飛行中のプレイヤーが高い場所にいる場合
execute as @a[tag=capi:is_flying,scores={capi:location_y=200..}] run say 空高く飛んでいます！

# スニーク中で移動していないプレイヤー
execute as @a[tag=capi:is_sneaking,scores={capi:velocity_xz=..10}] run say こっそり隠れています

# モバイルでプレイしているプレイヤーがブロックを破壊
execute as @a[tag=capi:is_mobile,tag=capi:break_block] run say モバイルでブロックを破壊しました！
```
