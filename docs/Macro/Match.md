...
```mcfunction
/execute as @a run scriptevent capi:actionbar 状態: <!match=[<!score=status>, §aオンライン, §e離席中, §cオフライン]>
```
...
Commander API のディメンションスコアを使用して現在のディメンションを表示します。
...
```mcfunction
/execute as @a run scriptevent capi:tell 難易度: <!match=[<!score=difficulty>, §aイージー, §eノーマル, §cハード, §4ハードコア]>
```
