...
:::warning 小数点の利用
Calc マクロでは小数を直接使用することはできません。  
代わりに、除算（分数）を使用して小数を表現してください。

| 小数 | 分数での表現 | 数式 |
|---|---|---|
| `0.5` | `1/2` または `5/10` | $\frac{1}{2} = 0.5$ |
| `0.25` | `1/4` または `25/100` | $\frac{1}{4} = 0.25$ |
| `0.125` | `1/8` または `125/1000` | $\frac{1}{8} = 0.125$ |
:::
